...
- Configurator must be running (`NODE_ENV=development npm start &`) for tools to work
...
cd inav-configurator && NODE_ENV=development npm start &
```
...
# If it fails, check:
# 1. Configurator is running (NODE_ENV=development npm start &)
