...
- `claude/agents/target-developer/scripts/simulate_pwm_roles.py` - F4/F7/AT32 equivalent of `check_dma_conflicts.py`'s role-cascade modeling: a faithful port of `pwmClaimTimer`/`pwmBuildTimerOutputList` plus per-family dmavar-indexed DMA-option resolution. Run with `--sweep` for a target's DMA-safe motorCount range, `--motor-count N` for a resolved-role table at one count, `--dmavar-override S8=1` (repeatable) to simulate a candidate `dmavar` change on a live entry (by its `TIM_USE_*` label) without editing target.c, to check a fix idea before applying it. See its own docstring and the lessons-learned entry below for the two "dead output" causes it distinguishes. **Known limitation:** `parse_target_c()` flattens `#if`/`#else`/`#ifdef` blocks into one array, which can silently corrupt position numbers (not just cause an obvious self-collision) on targets with conditional compilation in `timerHardware[]` -- see `make_pwm_shadow.sh` below for a workaround. **`USE_DSHOT_DMAR` (burst-mode DMA) aware:** on a target with `USE_DSHOT_DMAR`, a DMA stream is claimed once per *physical timer* (`tch->timCtx->dmaBurstRef`, shared across all 4 channels of one timer -- `timer_impl_stdperiph.c` `impl_timerPWMConfigDMABurst()`), not per channel, so the plain per-channel `DMA_STREAM_COLLISION` model is wrong for motor-vs-motor groups on these targets. The script now replays the real per-timer/per-channel ownership cascade (first channel of a timer still needing a claim attempts one using its own dmavar; success locks the timer and every later channel of that timer rides along for free; failure leaves the timer unlocked so the *next* channel of that same timer gets its own independent retry) and reports genuine cross-timer hazards as `BURST_STREAM_COLLISION` instead, while suppressing same-timer "collisions" that are actually the intended combined-request sharing. LED-vs-motor `DMA_STREAM_COLLISION` is untouched (LED strip never goes through the burst path). See the module docstring's numbered hazard list (item 3) for the full model, and `claude/projects/active/investigate-shared-tim-dma-request-lines/phase1-findings.md` (Phase 2 step 2) for the worked example that found all 6 previously-flagged DMAR targets (`FOXEERF405V2`, `NEXUSX`, `RADIOLINKF405`, `SKYSTARSF405V2`, `TUNERCF405`, `VANTAC_RF007`) were false positives under the naive model.
...
- `claude/agents/target-developer/scripts/classify_collisions.py` - Companion to `simulate_pwm_roles.py`: sweeps a list of F4/F7/AT32 targets across motor counts and classifies each `DMA_STREAM_COLLISION` (and, on `USE_DSHOT_DMAR` targets, `BURST_STREAM_COLLISION` -- see that script's entry above) hazard as CERTAIN or NOTICE. Severity is about the *loser*, not the group: motors claim DMA in strict position order (`pwm_mapping.c:pwmInitMotors`) and the first (lowest-position) claimant always wins, so CERTAIN requires the second-lowest position in the group to be < 4 -- an S1-S4 output actually going dead -- not just any member of the group being < 4. A motor-vs-motor pair where a low position "wins" against a higher one is NOT a hazard for the low side. Also surfaces two other patterns worth checking by hand: a motor colliding with a non-motor DMA claimer (e.g. LED_STRIP -- motors init first per `fc_init.c`, so the motor wins and the LED silently doesn't light) and same-(tim,ch) duplicate `DEF_TIM` entries collapsing to one, usually because `parse_target_c()` flattened a `#ifdef`-guarded build variant (not a real collision). Usage: `python3 classify_collisions.py <inav_root> <targets_file>`. Origin and worked example: `claude/projects/active/investigate-shared-tim-dma-request-lines/phase1-findings.md`.
...
- **`parse_target_c()`'s `#if`/`#ifdef` blindness can silently corrupt data, not just cause an obvious self-collision**: when a `target.c` declares two build variants with genuinely different DEF_TIM entries under `#if`/`#else` (not just a duplicate), flattening both into one array can produce a *plausible-looking but wrong* winner/loser pair (confirmed on `FF_PIKOF4`/`FF_PIKOF4OSD`) — worse than the already-known case where identical labels collapse into an obvious dead giveaway. Any target with conditional compilation inside `timerHardware[]` needs each variant isolated by hand (see `make_pwm_shadow.sh`) before trusting `simulate_pwm_roles.py`/`classify_collisions.py` output on it, not just the ones that visibly self-collide.
- **`USE_DSHOT_DMAR` invalidates naive per-channel DMA-stream-collision analysis for motor-vs-motor groups, in both directions (false positives AND, potentially, missed real collisions if you don't model the retry cascade)**: burst mode claims one DMA stream per *physical timer* (`tch->timCtx->dmaBurstRef`, on a context struct shared by all 4 channels of that timer -- `timer.c` `timerGetTCH()`), not per channel (`timer_impl_stdperiph.c` `impl_timerPWMConfigDMABurst()`). Only the first channel of a timer still needing a claim (`dmaBurstRef == NULL`) ever calls `dmaGetByTag()`/does the real ownership check, using its own dmavar; every later channel of that same timer, once it succeeds, rides along for free without ever consulting its own dmavar -- so a same-timer "collision" flagged by naive per-channel grouping is usually the intended combined-request sharing, not a bug. Two non-obvious corrections found while building the fix (`claude/projects/active/investigate-shared-tim-dma-request-lines/phase1-findings.md`, Phase 2 step 2): (1) a failed claim does NOT mean "the whole timer is dead" -- `dmaBurstRef` stays `NULL` on failure, so the *next* channel of that same timer gets its own fully independent retry with its own dmavar; and (2) a failed claim does NOT abort `pwmInitMotors()`'s loop either -- `motorConfigDshot()` returns a non-NULL port regardless of DMA success (only `port->configured` is gated), and `pwmMotorConfig()`'s return value is just `pwmPort != NULL`. Verified all 6 `USE_DSHOT_DMAR` targets flagged CERTAIN by a pre-DMAR-aware sweep (`FOXEERF405V2`, `NEXUSX`, `RADIOLINKF405`, `SKYSTARSF405V2`, `TUNERCF405`, `VANTAC_RF007`) were false positives once the real per-timer/per-channel cascade was modeled -- `simulate_pwm_roles.py`/`classify_collisions.py` now do this via a `BURST_STREAM_COLLISION` hazard class, processed in strict position order exactly like `pwm_mapping.c`'s real init loop.
