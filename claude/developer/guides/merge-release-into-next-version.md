...
**Use this procedure when:** You want to carry `release/9.x` changes forward into `maintenance-10.x` (or any older release into a newer one).
...
Branch from `maintenance-10.x`, not from `release/9.x`. This is the key difference from what GitHub's conflict resolver does.
...
Once `maintenance-10.x` contains the merge, GitHub detects that all commits from `release/9.1` are reachable from `maintenance-10.x` and closes the original `release/9.1` → `maintenance-10.x` PR as merged.
