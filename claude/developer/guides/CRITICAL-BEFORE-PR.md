...
- **Re-check bot/CI comments after every push, not just after PR creation**: fixing one bot's findings (e.g. Qodo) and pushing that fix can introduce new findings from a *different* bot (e.g. SonarCloud) on the exact lines just changed — this happened on inav-configurator PR #2671, where implementing Qodo's suggested fix introduced 5 new SonarCloud code smells (`var` instead of `let`/`const`, missed optional chaining) that went unnoticed because `check-pr-bots`/CI wasn't re-run after that specific push. Re-run `check-pr-bots` (or check the PR's SonarCloud/CI comment) after each push that touches code, not just once after the initial `gh pr create`.
- **Don't paste a bot-suggested code snippet verbatim if it doesn't match repo style**: Qodo's own suggested fix snippets are not guaranteed to follow this repo's conventions (e.g. they used `var` where this codebase's convention is `let`/`const`). Adapt suggested snippets to house style before committing — SonarCloud will flag the mismatch anyway, so it's cheaper to fix during implementation than to have it come back as a second round of findings.
