...
This demonstrates that we can undo the FC board alignment transformation
to get the true raw sensor orientation, regardless of FC settings.
"""
...
def main():
    if len(sys.argv) < 2:
        print("Usage: python3 test_inverse_transform.py <port>")
        print("Example: python3 test_inverse_transform.py /dev/ttyACM0")
        sys.exit(1)
    ...
    # Step 1: Read current FC alignment
    print("\n[1] Reading current FC alignment...")
    alignment = read_board_alignment(ser)
    if not alignment:
        print("    ERROR: Failed to read alignment")
        sys.exit(1)
    print(f"    FC alignment: roll={alignment['roll']:.1f}°, pitch={alignment['pitch']:.1f}°, yaw={alignment['yaw']:.1f}°")
    ...
    # Step 4: Set FC to 0,0,0 and verify
    print("\n[4] Setting FC to 0,0,0 to verify our calculation...")
    if not set_board_alignment(ser, 0, 0, 0):
        print("    WARNING: Set alignment failed")
    if not save_eeprom(ser):
        print("    WARNING: Save failed")

    print("    Please REBOOT the FC now, then press Enter...")
    input()
    ...
    print(f"\n[6] Restoring original alignment: roll={alignment['roll']:.1f}°, pitch={alignment['pitch']:.1f}°, yaw={alignment['yaw']:.1f}°...")
    if not set_board_alignment(ser, alignment['pitch'], alignment['roll'], alignment['yaw']):
        print("    WARNING: Set alignment failed")
    if not save_eeprom(ser):
        print("    WARNING: Save failed")
    print("    Please REBOOT the FC to apply the restored alignment")
