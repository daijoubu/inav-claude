...
def infer_mcu_family_from_target(target_name):
    ...
    # AT32 patterns (check FIRST before generic F4/F7/H7)
    if any(pattern in target_upper for pattern in ['AT32F435', 'AT32']):
        return 'AT32F435'

    # F4 patterns (but exclude AT32F435)
    if any(pattern in target_upper for pattern in ['F405', 'F411', 'F40']) and 'AT32' not in target_upper:
        return 'F4'
    if target_upper == 'F4':  # Only F4 by itself if not AT32
        return 'F4'
...
def check_firmware_hardware_match(hex_file, flash_layout):
    ...
    if not expected_family:
        print(f"  i  Target: {target_name} (MCU family unknown from filename)")
        return True

    if not detected_family:
        print(f"  i  Target: {target_name} (expected {expected_family}, but could not infer from detected layout)")
        return True

    if expected_family != detected_family:
        print()
        print("=" * 60)
        print("WARNING: FIRMWARE/HARDWARE MISMATCH DETECTED!")
        print("=" * 60)
...
def write_data(dev, block_num, data):
    ...
    After sending DNLOAD with data:
    - If state is dfuDNBUSY: wait for poll_timeout, then check status again
    - Final state should be dfuDNLOAD_IDLE (indicates write completed)
    """
    ...
    if status['state'] == DFU_STATE['dfuDNBUSY']:
        ...
        # After wait, should be in dfuDNLOAD_IDLE state (success)
        if status['state'] != DFU_STATE['dfuDNLOAD_IDLE']:
            raise Exception(f"Write failed, state={status['state']}")
        # If we are in dfuDNLOAD_IDLE, write succeeded - nothing to do
    else:
...
def flash_firmware(hex_file, mcu_type=None):
    ...
    # Find DFU device (supports both STM32 and AT32F435)
    print("Looking for DFU device (STM32 or AT32)...")
    dev = usb.core.find(idVendor=STM32_DFU_VID, idProduct=STM32_DFU_PID)
    
    # Also try Artery-Tech AT32F435
    if dev is None:
        AT32_DFU_VID = 0x2e3c
        dev = usb.core.find(idVendor=AT32_DFU_VID, idProduct=STM32_DFU_PID)

    if dev is None:
        raise Exception("No DFU device found (neither STM32 0483:df11 nor AT32 2e3c:df11). Put FC into DFU mode first.")
    ...
    try:
        ...
        # After erase, device may be in error state - clear it aggressively
        print("Clearing DFU status after erase...")
        time.sleep(0.1)
        clear_status(dev)
        print()
