#!/usr/bin/env python3
"""
Extract Table 9 "Alternate function mapping" from the STM32F405/F407 datasheet
using pdfplumber for accurate table extraction.

Reads pages 63-71 of stm32f405-datasheet.pdf and produces three output files:
...
PDF = (HERE / "../stm32f405-datasheet.pdf").resolve()
TSV_OUT = HERE / "alternate-functions.tsv"
MD_OUT = HERE / "alternate-functions.md"
INV_OUT = HERE / "af-by-function.txt"

# Table 9 spans PDF pages 63-71 (0-indexed: 62-70)
TABLE_PAGES = range(62, 71)
...
def clean_cell(val: str | None) -> str:
    ...
    pdfplumber uses \\n within a cell for multi-line content.
    Two cases:
      1. Continuation: "TIM2_CH1_\\nETR" or "SPI3_NSS/\\nI2S3_WS" — join directly
      2. Separate functions: "SPI3_NSS\\nI2S3_WS" — join with '/'
    Heuristic: if the previous fragment ends with '_', '-', or '/', it's a
    continuation; otherwise it's a new function name.
    """
    if val is None:
        return "-"
    ...
    for part in parts[1:]:
        if result.endswith(('_', '-', '/')):
            result = result + part        # continuation
        elif part.startswith(('_', '/')):
            result = result + part        # continuation from right
        else:
            result = result + '/' + part  # separate function name

    # Fix PDF layout spaces inside identifiers:
    # "TIM 5_CH1" → "TIM5_CH1", "TIM 2_CH1" → "TIM2_CH1"
    result = re.sub(
        r'\b(TIM|SPI|I2C|UART|USART|OTG|ETH|FSMC|SDIO|DCMI|CAN|ADC|DAC|RNG|RTC|JTMS|JTCK|JTRST|JTDO)\s+',
        r'\1',
        result,
    )
    # Fix stray spaces adjacent to underscores (PDF rendering artifacts like "ETH _MII" or "ETH_ MII")
    result = re.sub(r'_\s+', '_', result)
    result = re.sub(r'\s+_', '_', result)
    # Normalize remaining internal whitespace
    result = re.sub(r'\s+', ' ', result).strip()
    # Double underscores in datasheet (e.g. ETH_RMII__REF_CLK) — preserve as-is
    result = result.replace('__', '\x00').replace('_ _', '__').replace('\x00', '__')

    # Strip orphan trailing slash (truncated multi-line cell)
...
def parse_pdf() -> dict[str, dict[int, str]]:
    ...
    with pdfplumber.open(PDF) as pdf:
        for page_idx in TABLE_PAGES:
            ...
            # Find the header row (contains 'AF0', 'AF1' etc.)
            col_map: dict[int, int] = {}  # af_num -> column index
            for row in rows[:3]:
            ...
            # Find which column holds the pin name
            pin_col = None
            for row in rows[2:5]:
                for col_idx, cell in enumerate(row):
            ...
            if pin_col is None:
                pin_col = 1  # default: second column
...
def main() -> None:
    ...
    print(f"Reading {PDF.name} (pages 63-71)...")
    all_pins = parse_pdf()
    ...
    with MD_OUT.open("w") as f:
        f.write("# STM32F405/F407 Alternate Function Mapping (Table 9)\n\n")
        f.write("Source: STM32F405xx/STM32F407xx Datasheet DS8626, pages 63–71\n\n")
    ...
    with INV_OUT.open("w") as f:
        f.write("# STM32F405/F407 — Alternate Function to Pin Mapping\n")
        f.write("# Source: Table 9, DS8626 pages 63-71\n\n")
