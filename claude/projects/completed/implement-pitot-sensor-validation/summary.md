...
Successfully implemented GPS-based pitot sensor validation with automatic fallback to virtual airspeed. The implementation includes:
- Pitot failure detection using GPS groundspeed comparison (30%-200% thresholds)
- Automatic switch to GPS-based airspeed when pitot fails
- "PITOT FAIL" OSD warning display
- Hysteresis for stable transitions (1s failure detection, 2s recovery)
- Defensive airspeed clamping (100-20000 cm/s)
- Safe defaults (APA disabled by default, apa_pow = 0)
