...
**Root Cause:** Object-as-regex bug in `decompiler.js` - code stored entire hoisted activator entry object instead of extracting `.varName` property first. When object was used in string operation, it converted to `[object Object]` and caused "min" (from Math.min operations) to be inserted where spaces should be.

**Examples of Bug:**
- Expected: `const cond1 = ...`
- Actual: `constmincond1 = ...`

**Solution:** Extract `.varName` property from hoisted activator entries before using in string operations.
...
- Fix verified to resolve issue completely
- No regressions in other decompiler functionality
- All test cases passing

**Status:** PR #2541 merged into feature branch, ready for review before merge to main.

## Related

- **Branch:** `feature/js-programming-lc-highlighting`
- **PR:** [#2541](https://github.com/iNavFlight/inav-configurator/pull/2541)
- **Repository:** inav-configurator
- **Completion Report:** `manager/email/inbox/2026-01-29-2100-completed-investigate-decompiler-min-bug.md`
- **Workspace:** `claude/developer/workspace/js-minspace/`
