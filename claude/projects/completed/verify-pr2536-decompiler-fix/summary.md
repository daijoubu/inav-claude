...
**Result:** PR #2536 DOES fix the hoisting bug ✅

**Root Cause of Bug:** Decompiler incorrectly hoisted expressions depending on GVARs written inside conditional blocks, causing expressions to evaluate at parse time with stale values.

**How PR #2536 Fixes It:**
- Added robust GVAR dependency tracking in `activator_hoisting.js`
- New method `findWrittenGvars()` - Identifies GVARs modified in code blocks
- New method `readsFromWrittenGvar()` - Detects expression dependencies on runtime GVAR updates
- Prevents hoisting when it would break execution order

**Testing:**
- maintenance-9.x: Hoisting bug confirmed
- PR #2536 branch: Hoisting bug FIXED

**Recommendation:** PR #2536 can be merged with confidence - fixes both intended issues AND this hoisting bug.
...
- **Repository:** inav-configurator
- **Completion Report:** `manager/email/inbox/2026-01-29-2238-completed-check-pr2536-hoisting-bug.md`
- **Workspace:** `claude/developer/workspace/verify-pr2536-hoisting-bug/`
