...
1. **Build Linux x64 SITL locally** using the `build-sitl` skill (for glibc 2.35 compatibility)
2. Download other platform binaries from CI (Windows with cygwin1.dll, macOS, Linux arm64)
3. Create PR to update SITL in configurator
4. Wait for PR to merge before tagging

**Building SITL locally:**
```bash
/build-sitl
```
This skill guides you through building SITL on the maintenance-9.x branch. Build on Ubuntu 22.04 LTS (glibc 2.35) to ensure compatibility with all supported Ubuntu LTS releases.
