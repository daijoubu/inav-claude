...
- [ ] **CRITICAL: Verify nightly-build.yml includes maintenance-9.x**
  - Check `.github/workflows/nightly-build.yml` branch triggers
  - If maintenance-9.x NOT listed: Create PR to add it (see PR #11204)
  - CI runs on PR creation, NOT on merge - nightly builds are the only way to get complete artifacts
- [ ] **Trigger nightly build on maintenance-9.x**
  - Merge workflow PR or push any commit to maintenance-9.x
  - This triggers the nightly build with all merged changes
- [ ] **Wait for nightly build to complete and verify ALL jobs passed**
  - Check: `gh run list --repo iNavFlight/inav --limit 5`
  - If ANY job failed: **STOP** - fix before proceeding
  - Record the nightly release tag for artifact download
...
- [ ] If changed: **Build Linux x64 SITL locally** using `/build-sitl` skill (for glibc 2.35 compatibility)
- [ ] If changed: Download other platform binaries from CI (Windows with cygwin1.dll, macOS, Linux arm64)
- [ ] If changed: Create PR to update SITL in configurator
- [ ] If changed: Wait for PR to merge before proceeding

**Building Linux SITL locally:**
```bash
/build-sitl
```
Build on Ubuntu 22.04 LTS (glibc 2.35) to ensure compatibility with all supported Ubuntu LTS releases.
The CI-built Linux x64 SITL may require glibc > 2.35, which fails on Ubuntu 22.04.
...
### 2.1 Firmware Artifacts (from inav-nightly)
**IMPORTANT:** Download from the nightly release triggered by the maintenance-9.x push, NOT from PR CI runs!

```bash
# List recent nightly releases
gh release list --repo iNavFlight/inav-nightly --limit 5

# Download hex files and SITL from the correct nightly
gh release download <tag> --repo iNavFlight/inav-nightly --pattern "*.hex"
gh release download <tag> --repo iNavFlight/inav-nightly --pattern "sitl-resources.zip"
```

- [ ] Identify matching nightly build on inav-nightly (from maintenance-9.x push)
- [ ] Download all hex files from nightly
- [ ] Download sitl-resources.zip for SITL binaries
...
- [ ] Test basic configurator functionality (connect, read settings, etc.)
...
### 4.3 macOS Testing (if available)
- [ ] Mount DMG and launch configurator
- [ ] Start SITL
- [ ] Verify SITL connects and shows correct version

**Note:** Manual testing on at least Linux and Windows is required before creating release tags.
