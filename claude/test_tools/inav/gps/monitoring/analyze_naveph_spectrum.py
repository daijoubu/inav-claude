...
def analyze_blackbox_csv(csv_file):
    ...
    with open(csv_file, 'r') as f:
        ...
        # Check if navEPH column exists directly (INAV 9.0+)
        naveph_col = None
        navepv_col = None
        ...
        # Fall back to debug[7] extraction if direct columns not found
        debug7_col = None
        if naveph_col is None:
            for col in reader.fieldnames:
                if 'debug[7]' in col:
                    debug7_col = col
                    break

        if naveph_col is None and debug7_col is None:
            print("✗ Error: No 'navEPH' or 'debug[7]' column found in CSV")
        ...
        for row in reader:
            try:
                ...
                time = int(row.get('time (us)', row.get(' time (us)', row.get('loopIteration', 0))))

                # Get navEPH value
                if naveph_col is not None:
                    # Direct navEPH column (INAV 9.0+)
                    navEPH_cm = int(row[naveph_col])
                    navEPV_cm = int(row[navepv_col]) if navepv_col else 0
                    flags = 0
                else:
                    # Extract from debug[7] (older method)
                    debug7 = int(row[debug7_col])
