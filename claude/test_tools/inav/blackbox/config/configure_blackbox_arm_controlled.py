#!/usr/bin/env python3
"""
Configure Hardware Flight Controller for Blackbox Logging with Arm Control

Sets blackbox to log ONLY while armed (not continuously from boot).
...
def main():
    parser = argparse.ArgumentParser(description='Configure FC for blackbox logging (arm-controlled)')
    ...
    print("=" * 70)
    print("Hardware FC Blackbox Configuration (Arm-Controlled)")
    print("=" * 70)
    print()
    ...
    print(f"Arm Control:        0 (log only while armed, stop on disarm)")
    print()
    ...
    try:
        ...
        print()
        print("Configuring logging to start on ARM (stop on disarm)...")
        set_setting(api, 'blackbox_arm_control', 0)
        print(f"  ✓ blackbox_arm_control = 0 (log only while armed)")
        ...
        print("3. Erase blackbox flash")
        print("4. Run GPS injection test - logging will start on arm, stop on disarm")
        print()
